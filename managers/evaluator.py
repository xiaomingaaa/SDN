...
class Evaluator():
    ...
    def eval(self, save=False):
        ...
        # auc = calculate_auc_roc(pos_labels + neg_labels, pos_scores + neg_scores)
        # auc_pr = calculate_aupr(pos_labels + neg_labels, pos_scores + neg_scores)
        auc = calculate_auc_roc(torch.tensor(np.array(pos_scores + neg_scores)), torch.tensor(np.array(pos_labels + neg_labels)))
        auc_pr = calculate_aupr(torch.tensor(np.array(pos_scores + neg_scores)), torch.tensor(np.array(pos_labels + neg_labels)))
