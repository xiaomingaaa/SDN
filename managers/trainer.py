...
from model.dgl.layers import Regularization

from torchmetrics import MetricCollection, Accuracy, Precision, Recall, AUROC, AveragePrecision

calculate_auc_roc = AUROC(task="binary")
calculate_aupr = AveragePrecision(task="binary")

class Trainer():
    def __init__(self, params, graph_classifier, train, valid_evaluator=None):
        self.graph_classifier = graph_classifier
        self.reg = Regularization(self.graph_classifier, params.weight_decay)
    ...
    def train_epoch(self):
        ...
        dataloader = DataLoader(self.train_data, batch_size=self.params.batch_size, shuffle=False, num_workers=self.params.num_workers, collate_fn=self.params.collate_fn)
        self.graph_classifier.train()
        model_params = list(self.graph_classifier.parameters())
        for b_idx, batch in enumerate(dataloader):
            data_pos, targets_pos, data_neg, targets_neg = self.params.move_batch_to_device(batch, self.params.device)
            self.optimizer.zero_grad()
            score_pos, kl_loss_pos = self.graph_classifier(data_pos)
            score_neg, kl_loss_neg = self.graph_classifier(data_neg)
            # loss = self.criterion(score_pos, score_neg.view(len(score_pos), -1).mean(dim=1).unsqueeze(-1), torch.ones(score_pos.shape).to(device=self.params.device)) + 0.01*kl_loss_pos + 0.01*kl_loss_neg
            loss = self.criterion(score_pos, score_neg.view(len(score_pos), -1).mean(dim=1).unsqueeze(-1), torch.ones(score_pos.shape).to(device=self.params.device))
            # loss += self.reg(self.graph_classifier)
        ...
        auc = calculate_auc_roc(torch.tensor(np.array(all_scores)), torch.tensor(np.array(all_labels)))
        auc_pr = calculate_aupr(torch.tensor(np.array(all_scores)), torch.tensor(np.array(all_labels)))
        calculate_auc_roc.reset()
        calculate_aupr.reset()
    ...
    def train(self):
        self.reset_training_state()
        bar = tqdm(range(1, self.params.num_epochs + 1))
        for epoch in bar:
